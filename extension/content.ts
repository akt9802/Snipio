...
import type { BackgroundMessage, SnipSendResult, SnipToastKind } from "./types";

declare global {
  interface Window {
    __snipioLoaded?: boolean;
  }
}

const MIN_CSS_PX = 8;
const TOAST_MS = 2200;
const HOST_ATTR = "data-snipio";

type Rect = { left: number; top: number; width: number; height: number };

let snipHost: HTMLDivElement | null = null;
let toastTimer = 0;
...
function startSnip(dataUrl: string) {
  teardownSnip();
  ...
  const root = shadow.getElementById("root") as HTMLDivElement;
  const freeze = shadow.getElementById("freeze") as HTMLImageElement;
  freeze.src = dataUrl;
  const sel = shadow.getElementById("sel") as HTMLDivElement;
  const label = shadow.getElementById("label") as HTMLDivElement;
  const hint = shadow.getElementById("hint") as HTMLDivElement;
  const shade = shadow.getElementById("shade") as HTMLDivElement;
  ...
  (host as HTMLDivElement & { __snipioCleanup?: () => void }).__snipioCleanup = () => {
...
async function finishSnip(dataUrl: string, freeze: HTMLImageElement, rect: Rect) {
  ...
  const result = await chrome.runtime.sendMessage<BackgroundMessage, SnipSendResult>({
    type: "SNIP_CAPTURED",
    mime: "image/png",
    bytes,
  });
...
function teardownSnip() {
  if (!snipHost) return;
  const cleanup = (snipHost as HTMLDivElement & { __snipioCleanup?: () => void }).__snipioCleanup;
  cleanup?.();
  snipHost.remove();
  snipHost = null;
}
...
function localPoint(root: HTMLElement, event: MouseEvent) {
...
function applyRect(sel: HTMLElement, label: HTMLElement, rect: Rect) {
...
function showToast(text: string, kind: SnipToastKind) {
...
function overlayHtml() {
  return `
    ...
    <div id="root">
      ...
      <div id="shade"></div>
      <div id="hint">Drag to select a region · <span>Esc</span> to cancel</div>
      <div id="sel"><div id="label">0 × 0</div></div>
    </div>
  `;
}
